...
void setup() {
  ...
  timer1.setup([](boolean running, boolean ended, unsigned long timeElapsed) {
    if (running == true) {
      ...
      if ((currentAnalogInput1Value > 220 && newread == true) || (currentAnalogInput1Value > currentRead+50)){
        currentRead=currentAnalogInput1Value;
        newread = false;
        pulseCount++;
      }
      if (currentAnalogInput1Value < currentRead-10) {
        newread = true;
      }
    }
    else if (ended == true) {
